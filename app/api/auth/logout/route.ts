...
function isAuthCookieName(name: string) {
  return AUTH_COOKIE_NAMES.includes(name) || (name.startsWith("sb-") && name.includes("-auth-token"));
}
...
function clearCookie(response: NextResponse, name: string, request: Request) {
  for (const domain of COOKIE_DOMAINS) {
    ...
    response.cookies.set(name, "", domain ? { ...options, domain } : options);
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (sessionId) {
      await query(
        `update public.auth_sessions set is_active = false where session_id = $1::uuid`,
        [sessionId]
      );
    }
  } catch (error) {
    console.warn("Failed to deactivate practice candidate session", error);
  }
