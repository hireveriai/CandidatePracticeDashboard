...
// Mirrors calm-room/app/lib/aiUsageLog.ts and the recruiter-dashboard helper.
// Every OpenAI call records its billing metadata in `public.ai_audit_logs` so
// spend can be attributed to a feature, which the provider dashboard cannot do
// while one API key is shared across all apps and local development.
...
export type AiUsageEntry = {
  /** Dot-separated feature path, e.g. "practice.resume_ocr_pdf". */
  operation: string;
  model: string;
...
/**
 * Records one OpenAI call. Never throws and never blocks the caller. Inside a
 * request the insert is deferred past the response via `after()`; in scripts
...
export function logAiUsage(entry: AiUsageEntry) {
  ...
  try {
    // Required lazily rather than imported so `next/server` never becomes a
    // load-time dependency of this module, keeping it usable from any
    // non-Next context that reaches the same helpers.
